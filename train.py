    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for n_batch, (seq, label, file) in enumerate(self.val_loader):
                ...
                if self.logger:
                    wandb.log({"batch_val_loss": test_loss.item()})
            ...
            figure, axes = plt.subplots(nrows=1, ncols=8, figsize=(16,12), sharey=True)

            for i in range(8):
                t = torch.arange(0, seq.shape[1] / self.sample_rate, 1/self.sample_rate)
                v = seq[i]
                axes[i].plot(t.squeeze().numpy(), v.squeeze().numpy())
                axes[i].set_title(f'P:{pred_bin[i]}, L:{label[i]}')
        ...
        if self.cfg.save_model:
            ...
            if self.cfg.retrain:
                ...
                print("Saving to... ", self.base_model_file)
                torch.save(self.model.state_dict(),self.base_model_file)
